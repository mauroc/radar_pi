...
#include "RaymarineDigitalReceive.h"
...
//
//
#define SCALE_RAW_TO_DEGREES(raw) ((raw) * (double)DEGREES_PER_ROTATION / RAYMARINE_DIGITAL_SPOKES)
#define SCALE_DEGREES_TO_RAW(angle) ((int)((angle) * (double)RAYMARINE_DIGITAL_SPOKES / DEGREES_PER_ROTATION))
...
struct radar_line {
  ...
  uint16_t fills_5;
  uint8_t line_data[RAYMARINE_DIGITAL_MAX_SPOKE_LEN];
};
...
//
void RaymarineDigitalReceive::ProcessFrame(const uint8_t *data, size_t len) {
  ...
  const size_t packet_header_length = sizeof(radar_line) - RAYMARINE_DIGITAL_MAX_SPOKE_LEN;
  ...
  if (m_next_spoke >= 0 && spoke != m_next_spoke) {
    if (spoke > m_next_spoke) {
      ...
      m_ri->m_statistics.missing_spokes += RAYMARINE_DIGITAL_SPOKES + spoke - m_next_spoke;
    }
  }

  m_next_spoke = (spoke + 1) % RAYMARINE_DIGITAL_SPOKES;
...
SOCKET RaymarineDigitalReceive::PickNextEthernetCard() {
...
SOCKET RaymarineDigitalReceive::GetNewReportSocket() {
  SOCKET socket;
  wxString error;
...
SOCKET RaymarineDigitalReceive::GetNewDataSocket() {
  SOCKET socket;
  wxString error;
 ...
 */
void *RaymarineDigitalReceive::Entry(void) {
  int r = 0;
  ...
  LOG_VERBOSE(wxT("radar_pi: RaymarineDigitalReceive thread %s starting"), m_ri->m_name.c_str());
...
bool RaymarineDigitalReceive::UpdateScannerStatus(int status) {
  bool ret = true;
...
bool RaymarineDigitalReceive::ProcessReport(const uint8_t *report, size_t len) {
...
// this message ready for it to be read on 'm_receive_socket'. See the constructor in RaymarineDigitalReceive.h
// for the setup of these two sockets.

void RaymarineDigitalReceive::Shutdown() {
  if (m_send_socket != INVALID_SOCKET) {
...
wxString RaymarineDigitalReceive::GetInfoStatus() {
  wxCriticalSectionLocker lock(m_lock);
