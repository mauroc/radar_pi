...
#include "RaymarineDigitalControl.h"
...
RaymarineDigitalControl::RaymarineDigitalControl() {
  m_radar_socket = INVALID_SOCKET;
  m_pi = 0;
  m_ri = 0;
  m_name = wxT("RaymarineDigital");
}
...
bool RaymarineDigitalControl::Init(radar_pi *pi, RadarInfo *ri, NetworkAddress &ifadr, NetworkAddress &radaradr) {
  int r;
  int one = 1;

  m_addr = radaradr.GetSockAddrIn();
...
void RaymarineDigitalControl::logBinaryData(const wxString &what, const void *data, int size) {
  wxString explain;
...
bool RaymarineDigitalControl::TransmitCmd(const void *msg, int size) {
  if (m_radar_socket == INVALID_SOCKET) {
...
void RaymarineDigitalControl::RadarTxOff() {
...
void RaymarineDigitalControl::RadarTxOn() {
...
bool RaymarineDigitalControl::RadarStayAlive() {
  // Garmin radars don't need a ping
  return true;
}

bool RaymarineDigitalControl::SetRange(int meters) {
...
bool RaymarineDigitalControl::SetControlValue(ControlType controlType, RadarControlItem &item, RadarControlButton *button) {
  bool r = false;
